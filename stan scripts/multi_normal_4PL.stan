...
// The parameters accepted by the model. There are four main parameters
// in this model: alpha, beta, gamma, delta
// Here, we have introduced a log transform on both the beta and the gamma
// to ensure that their values cannot be negative.
parameters {
  matrix[1, n_subj] alpha_by_subj_raw;
  matrix[1, n_subj] log_beta_by_subj_raw;
  matrix[1, n_subj] log_gamma_by_subj_raw;
  matrix[1, n_subj] delta_by_subj_raw;
  
  vector[1] alpha_mean;
  vector[1] log_beta_mean;
  vector[1] log_gamma_mean;
  vector[1] delta_mean;

  vector<lower=0>[1] sd_alpha;
  vector<lower=0>[1] sd_log_beta;
  vector<lower=0>[1] sd_log_gamma;
...
transformed parameters {
  ...
  matrix[1, n_subj] log_beta_by_subj;
  matrix[1, n_subj] log_gamma_by_subj;
  matrix[1, n_subj] delta_by_subj;
  
  matrix[1, n_subj] beta_by_subj;
  ...
  theta_means = append_row(alpha_mean,
                           append_row(log_beta_mean,
                                      append_row(log_gamma_mean,
                                                 delta_mean
                                                 )
                                      )
                           );
  ...
  sd_theta = append_row(sd_alpha,
                        append_row(sd_log_beta,
                                   append_row(sd_log_gamma,
                                              sd_delta
                                              )
                                    )
                        );
  ...
  theta_by_subj_raw = append_row(alpha_by_subj_raw,
                                 append_row(log_beta_by_subj_raw,
                                            append_row(log_gamma_by_subj_raw,
                                                       delta_by_subj_raw
                                                       )
                                            )
                                );
  ...
  // Extract parameters by subject
  alpha_by_subj = to_matrix(theta_by_subj[1, ]);
  log_beta_by_subj = to_matrix(theta_by_subj[2, ]);
  log_gamma_by_subj = to_matrix(theta_by_subj[3, ]);
  delta_by_subj = to_matrix(theta_by_subj[4, ]);

  // Back-transform transformed parameters
  beta_by_subj = exp(log_beta_by_subj);
  gamma_by_subj = exp(log_gamma_by_subj);
...
model {
  ...
  log_beta_mean ~ normal(0, 3);
  log_gamma_mean ~ normal(0, 3);
  delta_mean ~ normal(0, 10);
  
  sd_alpha ~ weibull(2, 10);
  sd_log_beta ~ weibull(2, 1);
  sd_log_gamma ~ weibull(2, 1);
  sd_delta ~ weibull(2, 10);
  
  to_vector(alpha_by_subj_raw) ~ std_normal();
  to_vector(log_beta_by_subj_raw) ~ std_normal();
  to_vector(log_gamma_by_subj_raw) ~ std_normal();
  ...
  for (t in 1:n_y) {
	...
	y[t] ~ normal(delta_by_subj[1, subj_id[t]] +
                  ...
                  ( 1 + exp(- beta_by_subj[1, subj_id[t]] *
                                (x[t] - gamma_by_subj[1, subj_id[t]])) ),
                  sd_y);
                  }
}
